...
std::vector<int> PittVC(int numNodes, std::vector<std::tuple<int, int>>& Edges, std::vector<int>& node_weights){
    // Pitt für Vertex Cover (gewichtete Graphen)
    int u,v,wv,wu;
    std::vector<int> VCP(numNodes, 0); //Vektor für Vertex Cover
     for( int i = 0; i < Edges.size(); i++){
        ...
        if (VCP[u] == 0 && VCP[v] == 0){
            ...
            std::random_device rd;
            std::mt19937 gen(rd());
            std::uniform_real_distribution<> dis(0.0, 1.0); // Zufallszahlen im Bereich [0.0, 1.0]
            double randomValue = dis(gen);

            if (randomValue < (wu/(wu+wv))){
                VCP[u] = 1;
            } else{
                VCP[v] = 1;
            }
            ...
            while (std::getline(inputCsv, line)) {
                ...
                for (int i = 0; i < 1; ++i) {
                   ...
                   // const auto& vcVector = result.second;
                    std::vector<int> vcVector = PittVC(graphData.n, graphData.edges, node_weights);
